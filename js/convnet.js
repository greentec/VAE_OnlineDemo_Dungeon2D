...
(function(global) {
  ...
  ReshapeLayer.prototype = {
    forward: function(V) {
      ...
      // for (var i = 0; i < this.out_sx; i += 1) {
      //   for (var j = 0; j < this.out_sy; j += 1) {
      //     for (var k = 0; k < this.out_depth; k += 1) {
      //       // A.set(i, j, k, V.get(i + j * this.out_sx + k * this.out_depth));
      //       // A.set(i, j, k, V.get(i, j, k));
      //       A.w[i+j*this.out_sx+k*this.out_depth] += V.w[i+j*this.out_sx+k*this.out_depth];
      //     }
      //   }
      // }
      // var len = this.out_sx * this.out_sy * this.out_depth;
  ...
  Net.prototype = {
    ...
    makeLayers: function(defs) {
      ...
      for(var i=0;i<defs.length;i++) {
        ...
        switch(def.type) {
          ...
          case 'reshape': this.layers.push(new global.ReshapeLayer(def)); break;
    ...
    fromJSON: function(json) {
      ...
      for(var i=0;i<json.layers.length;i++) {
        ...
        if(t==='reshape') { L = new global.ReshapeLayer(); }
        L.fromJSON(Lj);
        this.layers.push(L);
      }
    }
